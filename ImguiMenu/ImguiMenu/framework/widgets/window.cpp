...
bool scrollbar_ex(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, draw->get_clr(clr->window.child), window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, draw->get_clr(clr->accent), var->style.scrollbar_rounding);
...
bool c_gui::begin(std::string_view name, bool* p_open, window_flags flags)
{
    ...
    if (first_begin_of_the_frame && !window->SkipRefresh)
    {
        ...
        float needed_scroll_clamp = calc_next_scroll_from_scroll_target_and_clamp(window, true).y;

        gui->easing(anim->scroll, needed_scroll_clamp, 16.f, dynamic_easing);

        if (window->Scroll.y != needed_scroll_clamp)
        {
            if (!(flags & ImGuiWindowFlags_NoScrollWithMouse) && window->Scroll.y != needed_scroll_clamp)
                window->Scroll.y = roundf(anim->scroll);
        }
        else
        {
