...
widget_t c_gui::register_item(std::string_view name, const ImVec2& size)
{
    ...
    data->visible = gui->is_rect_visible(data->rect);
    data->hovered = data->rect.Contains(g.IO.MousePos);

    gui->item_size(data->rect);
    gui->item_add(data->rect, data->id);
...
void c_gui::set_style()
{
    ...
    style->window_shadow_size = 0;
    style->window_rounding = SCALE(var->window.rounding);
    style->window_padding = SCALE(var->window.indent);
    style->item_spacing = SCALE(var->window.items_spacing);
    style->scrollbar_size = SCALE(var->window.size_scrollbar);
    style->scrollbar_border_padding = SCALE(var->window.margins_scrollbar);
    style->scrollbar_content_padding = SCALE(var->window.padding_scrollbar);
...
void c_gui::draw_decorations()
{
    ...
    draw_background_blur(gui->window_drawlist(), var->device, var->context, var->style.window_rounding);
    
    draw->rect_filled(drawlist, pos, pos + size, draw->get_clr(clr->window.background, 0.9f), var->style.window_rounding);
    draw->rect_filled(drawlist, pos + SCALE(elements->section.size.x, 0), pos + size, draw->get_clr(clr->window.bar), var->style.window_rounding);

    draw->line(drawlist, pos + SCALE(var->gui.icon.x, elements->section.size.x), pos + SCALE(var->gui.icon.x, elements->section.size.x) + SCALE(var->gui.line, 0), draw->get_clr(clr->window.line));
    draw->text_clipped(gui->window_drawlist(), font->get(icon_data, 22), pos + SCALE(var->gui.icon), pos + SCALE(var->gui.icon) + SCALE(var->gui.icon.y, var->gui.icon.x), draw->get_clr(clr->accent), "A", NULL, NULL, ImVec2(0.5f, 0.5f));

    draw->text_clipped(gui->window_drawlist(), font->get(icon_data, 8), pos, pos + ImVec2(SCALE(elements->section.size.x), size.y - SCALE(elements->section.padding.y * 2.5 + 35)), draw->get_clr(clr->widgets.text), "G", NULL, NULL, ImVec2(0.5f, 1.f));
}
std::string c_gui::lang(std::string ru, std::string en)
{
    return var->gui.selected_language == 0 ? ru : en;
}
