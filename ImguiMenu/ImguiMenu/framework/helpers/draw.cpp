...
void c_draw::separator()
{
    ImGuiContext& g = *GImGui;
    ImDrawList* drawlist = GetWindowDrawList();

    if (gui->content_avail().y + gui->get_window()->ScrollMax.y > 0)
        draw->rect_filled(drawlist, ImVec2(gui->window_pos().x, g.LastItemData.Rect.Max.y), ImVec2(gui->window_pos().x + gui->window_width(), g.LastItemData.Rect.Max.y + var->style.item_spacing.y), draw->get_clr(clr->window.rect));
}